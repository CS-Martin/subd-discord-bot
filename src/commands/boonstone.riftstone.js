...
export const attendance_boonstone_riftstone_command = {
    data: (() => {
        const builder = new SlashCommandBuilder()
            .setName('boonstone-riftstone-attendance')
            .setDescription('Use this command to record your attendance for attending Boonstone Riftstone.');
    ...
    async execute(interaction) {
        console.log('Starting Boonstone/Riftstone Attendance Command...');
        await interaction.deferReply();

        try {
            const excel_data = await googleSheetService.readSheet(process.env.SPREADSHEET_ID, 'Daily Tally!A1:E100');
            ...
            for (let i = 1; i <= 6; i++) {
                ...
                const current_points = parseInt(excel_data[foundUserIndex][4], 10) || 0;
                excel_data[foundUserIndex][4] = current_points + 5;
            }
            ...
            await googleSheetService.updateSheet(
                ...
                `Daily Tally!A1:E${excel_data.length}`,
                excel_data
            );

            const embed = new EmbedBuilder()
                .setTitle('Successfully Recorded Boonstone/Riftstone Attendance!')
            ...
            console.error('Error in Boonstone/Riftstone Attendance Command:', error);
