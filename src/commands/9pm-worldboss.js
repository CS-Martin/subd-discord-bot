...
export const attendance_9pm_command = {
    data: (() => {
        const builder = new SlashCommandBuilder()
            .setName('9pm-worldboss-attendance')
    ...
    async execute(interaction) {
        console.log('Starting 9PM World Boss Attendance Command...');
        await interaction.deferReply();

        try {
            const excel_data = await googleSheetService.readSheet(process.env.SPREADSHEET_ID, 'Daily Tally!A1:C100');
            ...
            for (let i = 1; i <= 6; i++) {
                ...
                const current_points = parseInt(excel_data[foundUserIndex][2], 10) || 0;
                excel_data[foundUserIndex][2] = current_points + 2;
            }
            ...
            await googleSheetService.updateSheet(
                ...
                `Daily Tally!A1:C${excel_data.length}`,
                excel_data
            );

            const embed = new EmbedBuilder()
                .setTitle('Successfully Recorded 9:00 PM Attendance!')
            ...
            console.error('Error in 9PM World Boss Attendance Command:', error);
