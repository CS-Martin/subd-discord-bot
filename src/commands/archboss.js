...
export const attendance_archboss_command = {
    data: (() => {
        const builder = new SlashCommandBuilder()
            .setName('archboss-attendance')
            .setDescription('Use this command to record your attendance for attending Archboss.');
    ...
    async execute(interaction) {
        console.log('Starting Archboss Attendance Command...');
        await interaction.deferReply();

        try {
            const excel_data = await googleSheetService.readSheet(process.env.SPREADSHEET_ID, 'Daily Tally!A1:J100');
            ...
            for (let i = 1; i <= 6; i++) {
                ...
                const current_points = parseInt(excel_data[foundUserIndex][9], 10) || 0;
                excel_data[foundUserIndex][9] = current_points + 5;
            }
            ...
            await googleSheetService.updateSheet(
                ...
                `Daily Tally!A1:J${excel_data.length}`,
                excel_data
            );

            const embed = new EmbedBuilder()
                .setTitle('Successfully Recorded Archboss Attendance!')
            ...
            console.error('Error in Archboss Attendance Command:', error);
